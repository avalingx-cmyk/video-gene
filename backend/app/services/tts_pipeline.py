...
async def generate_tts_for_segment(
    ...
    """
    Generate TTS audio for a segment narration using Groq Orpheus.
    Returns dict with url, local_path, duration_seconds, and status.
    """
    ...
    for attempt in range(TTS_MAX_RETRIES):
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                ...
                logger.info(f"Generated TTS: {local_path} ({len(resp.content)} bytes, {duration:.1f}s)")
    ...
    return {
        "status": "failed",
        "error": f"TTS failed after {TTS_MAX_RETRIES} retries. Last error: {last_error}",
        "url": None,
        "local_path": None,
